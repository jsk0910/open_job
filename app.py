...
def showJob(recommend_jobs, similarity_jobs):
    ...
    st.write('<style>div.row-widget.stRadio > div{flex-direction:row;}</style>', unsafe_allow_html=True)
    st.session_state.selected_job, st.session_state.jobs
...
def main():
    ...
    if uploaded_file:
        st.session_state.recommend_jobs = jaccard.recommend_job(uploaded_file)
        ...
        #         st.session_state.gangso, st.session_state.recommend_company = corp.find_company(st.session_state.clicked_regionCd, st.session_state.clicked_jobCd)
